...
function init(db) {
    ...
    router
        ...
        try {
            ...
            const query = {"userID": parseInt(user_id)}
            const users_messages = await messages.getMessages(query)
    ...
    router
        ...
        .get(async (req, res) => { 
            ...
            const user_info = await users.get(user_id)
            if (!user_info){
                res.status(401).json({
                    status: 401,
                    message: "User not found"
    ...
    router
        ...
        router
        .route("/user/:user_id(\\d+)/admin/:user_id2(\\d+)/removeAdmin")
        ...
        .post(async (req, res) => {
            try{
                ...
                const setAdmin = await users.removeAdmin(userId2)
                ...
                res.status(200).json({
                    status: 200,
                    message: "User succesfully removed from admin"
    ...
    router
        .route("/user/:user_id(\\d+)/admin/:user_id2(\\d+)")
        .delete(async (req,res)=>{
            try{
                console.log("TEST 1 ")
                const userId = req.params.user_id;
                ...
                const deletedUserId = await users.delete(userId2);
